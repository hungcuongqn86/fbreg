...
namespace GenCode
{
	public class RegClone
	{
		...
		public async Task<bool> RegFaceBook(bool useapi = false)
		{
			bool _status = false;
			await Task.Run(async delegate ()
			{
				try
				{
					...
					WaitLoading();
					Delay(1000);
					string tdAdsInfo = "//td[@data-testid='all_accounts_table_account_id_cell']";
					ReadOnlyCollection<IWebElement> tdAdsInfoObjs = this._driver.FindElements(By.XPath(tdAdsInfo));
					if (tdAdsInfoObjs.Count > 0)
					{
						_ads_id = tdAdsInfoObjs.First().Text;
					}
		...
		private async Task<bool> OverLimitHandle()
		{
			...
			await Task.Run(delegate ()
			{
				try
				{
					...
					if (flag14)
					{
						...
						string _rNamePage = "HTCFunny " + new Random().Next(100000, 999999).ToString();
		...
		public Task<bool> shareAdsToVia(string viaid)
		{
			try
			{
				...
				string post = this.PostData(null, url, postdata, this._contentType, this._userAgent2, _cookie);
				// this.log(post);
				return Task.FromResult(true);
			}
			catch (Exception ex)
